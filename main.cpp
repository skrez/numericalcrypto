...
void example_discrete_laplacian();

double** discrete_laplacian(int n);
...
void example_discrete_laplacian()
{
    print_example_banner("Example: Discrete Laplacian");
    ...
    */
    ...
    double **l = discrete_laplacian(10);
    for (int i = 0; i < 10; i++){
        for (int j = 0; j < 10; j++)
            cout << l[i][j] << " ";
        cout << endl;
    ...
    */
    int num_vecs = 10;
    auto encoded_coefficients = new vector<Plaintext>[num_vecs];
    cout << "Encoding plaintext coefficients: ";
    for (int j = 0; j < num_vecs; j++)
    for (int i = 0; i < 10; i++)
    {
        encoded_coefficients[j].emplace_back(encoder.encode(l[j][i]));
        //cout << to_string(discrete_laplacian[0][i]).substr(0,6) << ((i < 9) ? ", " : "\n");
        cout << l[j][i] << ((i < 9) ? ", " : "\n");
        //cout << endl;
    ...
    */
    auto encrypted_products = new vector<Ciphertext>[num_vecs];
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        */
        encrypted_products[j].resize(encrypted_products[j].size() +1);
        evaluator.multiply_plain(encrypted_rationals[i], encoded_coefficients[j][i], encrypted_products[j][i]);
    }
    cout << "Done" << endl;
    ...
    Ciphertexts.
    */
    auto encrypted_result = new Ciphertext[num_vecs];
    cout << "Adding up all 10 ciphertexts: ";
    for (int j = 0; j < num_vecs; j++)
    evaluator.add_many(encrypted_products[j], encrypted_result[j]);
    cout << "Done" << endl;
    ...
    */
    cout << "Noise budget in result 0: "
        << decryptor.invariant_noise_budget(encrypted_result[0]) << " bits" << endl;

    /*
    Decrypt, decode, and print result.
    */
    Plaintext plain_result[num_vecs];
    cout << "Decrypting result: ";
    for (int j = 0; j < num_vecs; j++){
        decryptor.decrypt(encrypted_result[j], plain_result[j]);
        double result = encoder.decode(plain_result[j]);
        cout << result << " ";
    }
    cout << endl;
}
